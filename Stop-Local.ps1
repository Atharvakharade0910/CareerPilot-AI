...
foreach ($port in @(8010,3010)) {
    ...
    if (!($owner.CommandLine.Replace('/', '\').Contains($PSScriptRoot) -or $owner.ExecutablePath.Replace('/', '\').StartsWith($PSScriptRoot))) { Write-Warning "Leaving port $port alone: owner is not identifiable as this project."; continue }
